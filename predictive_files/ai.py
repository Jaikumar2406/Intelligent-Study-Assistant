...
def extract_text_from_file1(file_path):
    """
    Processes a file (PDF or image), extracts text using OCR or PDF parsing,
    checks for a valid code 'C109511(022)', and if valid, processes it with LLM
    and saves the final text into 'ai.txt'.
    """
    ...
    if "C109511(022)" not in text:
        print("'C109511(022)' not found in the document. Skipping save.")
        return None
    
    processed_text = llm(text)

    save_in_txt(processed_text, 'ai.txt')
    print("saved")
