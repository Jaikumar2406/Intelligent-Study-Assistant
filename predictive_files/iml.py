...
def extract_text_from_file2(file_path):
    """Processes a file (PDF or image), extracts text using OCR or PDF parsing,
    checks for a valid code 'C109512(022)', and if valid, processes it with LLM
    and saves the final text into 'iml.txt'.
    """
    ...
    if "C109512(022)" not in text:
        print("'C109512(022)' not found in the document. Skipping save.")
        return None
    
    processed_text = llm(text)

    save_in_txt(processed_text, 'iml.txt')
